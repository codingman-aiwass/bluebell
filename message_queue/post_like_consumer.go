...
func NewLikeProcessor(brokers []string, topic string, maxRetries int) *LikeProcessor {
	reader := kafka.NewReader(kafka.ReaderConfig{
		Brokers:     brokers,
		Topic:       topic,
		GroupID:     "post_like_event_consumer_group",
...
func (lp *LikeProcessor) Start(ctx context.Context) {
	go lp.consumeMessages(ctx)
	go lp.processLikes(ctx)
...
func (lp *LikeProcessor) consumeMessages(ctx context.Context) {
	for {
...
func (lp *LikeProcessor) processLikes(ctx context.Context) {
	for {
		select {
		case msg := <-lp.messages:
			...
			if err := lp.handleLike(likeEvent); err != nil {
...
// 处理点赞事件，带有不同的处理策略
func (lp *LikeProcessor) handleLike(event PostLikeEvent) error {
	var err error
	for i := 0; i <= lp.maxRetries; i++ {
		...
		case "like":
			...
			// 处理点赞逻辑
			err = likePostDatabaseOperation(event) // 执行点赞操作
		case "none":
			zap.L().Info(fmt.Sprintf("User %d cancel liked post %d at %s\n", event.UserId, event.PostId, event.Timestamp))
			// 处理取消点赞逻辑
			err = cancelLikePostDatabaseOperation(event)
		default:
...
// 处理死信队列中的事件
func (lp *LikeProcessor) handleDeadLetters(ctx context.Context) {
	for {
		select {
...
// 点赞数据库操作
func likePostDatabaseOperation(event PostLikeEvent) error {
...
// 取消点赞数据库操作
func cancelLikePostDatabaseOperation(event PostLikeEvent) error {
	oValue := mysql_repo.VoteRepository.FindOne(sqls.DB(), sqls.NewCnd().Where("user_id = ?", event.UserId).Where("target_id = ?", event.PostId))
	if oValue == nil {
		// 说明没有点过赞/踩，没必要执行操作
		return nil
